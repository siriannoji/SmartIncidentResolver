...
# Helper to read different document formats
...
# Chatbot logic (simple match-based for now)
...
# Updated 'Search > Project information' section
    ...
    if search_type == "Incident data":
        ...
        if query and st.button("Search Incidents"):
            ...
            # Use first available CSV for demo

            incident_files = [f for f in os.listdir(INCIDENT_DIR) if f.endswith('.csv')]
            ...
            if not incident_files:
                ...
                df = load_incident_csv(os.path.join(INCIDENT_DIR, incident_files[0]))

                index, embeddings = create_index_from_descriptions(df['description'].tolist())

                results = search_incidents(query, df, index, embeddings)

                resolutions = results['resolution'].tolist()

                summary = summarize_texts(resolutions)
